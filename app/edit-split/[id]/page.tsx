...
export default function EditSplitPage() {
  ...
  useEffect(() => {
    async function loadSplit() {
      ...
      const s = split as unknown as Split;
      ...
      setPerfumeVariant(s.perfume?.variant ? s.perfume.variant.split(",").map((v: string) => v.trim()).filter(Boolean) : []);
      setDescription(s.description ?? "");
      setConcentration((s.perfume?.concentration as Concentration) ?? "");
      setBottleSize(String(s.bottle_size_ml));
      setBatchCode(s.batch_code ?? "");
      setIsReadyStock(s.is_ready_stock);
      ...
      setTopNotes(s.perfume?.top_notes ?? []);
  ...
  async function handleSubmit(e?: FormEvent) {
    ...
    try {
      ...
      if (batchCodePhoto) {
        ...
        if (!url) {
          setError("Gagal mengunggah foto batch code");
          setLoading(false);
          return;
        }
        batchCodePhotoUrl = url;
      }

      let decantVideoUrl = existingDecantVideo;
      if (decantVideo) {
        const url = await uploadImage(
          supabase,
          decantVideo,
        ...
        if (url) decantVideoUrl = url;
      }

      const validVariants = variants
        .filter((v) => Number(v.size_ml) > 0 && Number(v.price) > 0 && Number(v.stock) > 0)
        .map((v) => ({
